...
from gestor_portafolio import (listar_portafolios, leer_portafolio,
                                guardar_aporte, activar_portafolio)
...
def inicializar_aportes(archivo, portafolio):
    ...
    print(f"   {'Activo':<12} {'Peso':>6} {'Monto COP':>14} "
    ...
    for activo, peso in composicion.items():
        ...
        print(f"   {activo:<12} {peso*100:>5.1f}% "
              f"${monto_cop:>13,.0f} "
              f"${precio:>11,.2f} "
              f"{fracciones:>11.4f}")
...
def registrar_nuevos_aportes(archivo, portafolio):
    ...
    while continuar:
        ...
        try:
            ...
            guardar_aporte(archivo, aporte)
            print(f"   ✅ {activo}: {fracciones:.4f} fracciones @ "
                  f"${precio_usd:,.2f} USD")
...
def calcular_estado_actual(portafolio):
    ...
    for activo, fracciones in fracciones_por_activo.items():
        ...
        rentabilidad = (ganancia / total_inv_real * 100
                       ) if total_inv_real > 0 else 0
    ...
    rent_global     = (ganancia_global / resumen['total_invertido_real'] * 100
                      ) if resumen['total_invertido_real'] > 0 else 0
...
def detectar_deriva(portafolio, resultados):
    ...
    composicion        = portafolio.get('composicion', {})
    alertas_rebalanceo = []
    datos              = {k: v for k, v in resultados.items()
                         if k != '_resumen'}
    ...
    for activo, datos_activo in datos.items():
        ...
        if abs(deriva) > 5:
            sugerencia = ("⚠️ Subió mucho — en próximo DCA no compres más"
                         if deriva > 0 else
                         "⚠️ Bajó — en próximo DCA pon más aquí")
