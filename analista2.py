...
def calcular_senales(ticker):
    try:
        ...
        tendencia_mensual = (
            1 if len(close_m) >= 3 and close_m.iloc[-1] > close_m.iloc[-3] else 0
        )
        tendencia_semanal = (
            1 if len(close_w) >= 4 and close_w.iloc[-1] > close_w.iloc[-4] else 0
        )
        ...
        if rsi_d < 40:
            score += 2
            razones.append(f"RSI diario sobrevendido ({rsi_d})")
        elif rsi_d < 50:
            score += 1
            razones.append(f"RSI diario en zona de valor ({rsi_d})")

        if precio_actual <= banda_inf * 1.02:
            score += 2
            razones.append("Precio en/cerca de banda inferior Bollinger")

        if precio_actual <= ma50 * 1.01:
            score += 1
            razones.append("Precio en soporte MA50")

        if cruce_alcista:
            score += 2
            razones.append("Cruce alcista MACD")
        ...
        if tendencia_mensual == 1:
            score += 1
            razones.append("Tendencia mensual alcista")
        if tendencia_semanal == 1:
            score += 1
            razones.append("Tendencia semanal alcista")
        if ratio_volumen > 1.5:
            score += 1
            razones.append(f"Volumen elevado ({ratio_volumen}x promedio)")

        senal = (
            "🟢 ENTRAR" if score >= 6 else "🟡 VIGILAR" if score >= 3 else "⚪ NEUTRAL"
        )
...
def revisar_portafolio():
    ...
    print(
        f"   Portafolio: {portafolio['nombre']} ({portafolio['perfil']}) — {portafolio['propietario']}"
    )
    ...
    for ticker in todos:
        ...
        print(
            f"{senal['senal']} | RSI: {senal['rsi_diario']} | "
