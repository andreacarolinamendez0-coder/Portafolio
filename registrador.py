...
from gestor_portafolio import (leer_portafolio_activo, listar_portafolios,
...
def capturar_estado_hoy():
    ...
    for p_info in portafolios:
        ...
        inf_anual  = portafolio.get('inflacion_col',
                     macro.get('inf_col', 4.90))
        ...
        for activo, fracciones in fracciones_por_activo.items():
            ...
            inv_real     = sum(
                a['monto_cop'] / (1 + inf_anual/100) **
                max((datetime.now() - datetime.strptime(
                    a['fecha'], "%Y-%m-%d")).days / 365.25, 0)
            ...
            print(f"   {emoji} {portafolio['nombre']} | {activo}: "
        ...
        if guardado:
            ...
            print(f"      Invertido: ${total_inv:,.0f} | "
