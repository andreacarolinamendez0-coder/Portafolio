...
def calcular_retornos_reales(panel, activos):
    ...
    print(f"   Con momentum    : {len(ret_ponderado)} meses "
...
def optimizar_portafolio(ret_real, panel, perfil, risk_free, inversion_inicial):
    ...
    resultado = minimize(objetivo, w0, method='SLSQP',
...
def proyectar_con_dca(ret_port, inversion_inicial, aporte_periodico,
                      frecuencia_meses, horizonte_años, inflacion_col,
                      tasa_cdt, n_sim=500):
    """
    ...
    def percentiles(res):
        r = sorted(res)
        return (r[int(n_sim * 0.10)],
                r[int(n_sim * 0.50)],
                r[int(n_sim * 0.90)])

    return (percentiles(resultados_global),
...
def generar_reporte(pesos, inversion_inicial, ret_real, perfil, horizonte,
                    risk_free, inflacion_col, tasa_cdt,
                    aporte_periodico=0, frecuencia_meses=1):
    ...
    for años in años_proyeccion:
        ...
        print(f"   {'─'*60}")
        print(f"   {'Portafolio (sin aportes)':<28} "
              f"${sin_dca[0]:>10,.0f} "
              f"${sin_dca[1]:>10,.0f} "
              f"${sin_dca[2]:>10,.0f}")
        ...
        if aporte_periodico > 0:
            ...
            print(f"   {'Portafolio + DCA '+freq:<28} "
                  f"${con_dca[0]:>10,.0f} "
                  f"${con_dca[1]:>10,.0f} "
                  f"${con_dca[2]:>10,.0f}")

        print(f"   {'CDT (referencia)':<28} "
              f"{'':>12} "
              f"${cdt_real:>10,.0f} "
              f"{'':>12}")

        if aporte_periodico > 0:
            print(f"   {'CDT + aportes':<28} "
                  f"{'':>12} "
...
# ── ACTUALIZAR DATOS ANTES DE ANALIZAR ──
    ...
    archivo       = portafolio_activo.get('archivo') or \
    ...
    if aporte_dca > 0:
        freq_texto = {1: "mensual", 3: "trimestral", 12: "anual"}
        print(f"   Aporte DCA        : ${aporte_dca:,.0f} COP "
    ...
    inflacion_col_actual = float(
        pd.read_parquet(f"{CARPETA_MACRO}/inflacion_col.parquet")['Inflacion_COL'].iloc[-1]
    )
    ...
    generar_reporte(pesos, inversion_inicial, ret_real, perfil_activo,
                    horizonte=horizonte, risk_free=risk_free,
