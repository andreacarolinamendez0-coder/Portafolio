...
+ '</div>'
    ...
    contenido = (
        ...
        f'<script>'
        f'<script>'
f'function mercadoAbiertoAhora(){{'
f'  const ahora = new Date();'
f'  const col = new Date(ahora.toLocaleString("en-US", {{timeZone:"America/Bogota"}}))'
f'  const dia = col.getDay();'  
f'  const hora = col.getHours() * 60 + col.getMinutes();'
f'  return dia >= 1 && dia <= 5 && hora >= 570 && hora <= 960;'  
f'}}'
f'const MERCADO_ABIERTO = mercadoAbiertoAhora();'
f'console.log("Mercado abierto:", MERCADO_ABIERTO);'
