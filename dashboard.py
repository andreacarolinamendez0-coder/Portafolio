...
def grafica_trm(trm_hist):
    ...
    try:
        ...
        for termino in ["dolar peso colombiano TRM", "tasa cambio Colombia hoy"]:
            try:
                ...
                for item in ET.fromstring(r.content).findall(".//item")[:2]:
                    ...
                    if t is not None:
                        noticias.append(
                            f"- {t.text[:120]} ({d.text[:16] if d is not None else ''})"
                        )
            except Exception as e:
        ...
        tendencia = (
            "alcista"
            if trm_hoy > trm_ma7 > trm_ma30
            else "bajista" if trm_hoy < trm_ma7 < trm_ma30 else "lateral"
        )

        # Volatilidad: desviación estándar últimos 30d
        volatilidad = float(trm_series.tail(30).std())
        vol_nivel = (
            "alta" if volatilidad > 80 else "moderada" if volatilidad > 40 else "baja"
        )

        # Posición relativa en el rango de 90 días (0% = mínimo, 100% = máximo)
        rango_90d = trm_max90 - trm_min90
        posicion_rango = (
            ((trm_hoy - trm_min90) / rango_90d * 100) if rango_90d > 0 else 50
        )

        analisis_trm = anthropic_chat(
            [
                {
                    "role": "user",
                    "content": f"Eres analista cambiario senior del mercado colombiano. "
...
def grafica_torta(pesos, titulo):
    ...
    fig = go.Figure(
        go.Pie(
            ...
            hole=0.55,
            marker=dict(colors=cf[:n], line=dict(color=cb[:n], width=3)),
    ...
    fig.update_layout(
        ...
        font=dict(color="#a1a1a6", family="DM Sans"),
        height=300,
        margin=dict(l=10, r=10, t=20, b=10),
        showlegend=False,
        hoverlabel=dict(
            bgcolor="rgba(12,12,12,0.97)",
...
def grafica_ganancias(tiempo_real):
    ...
    act = [p["activo"] for p in pos]
    gan = [p["ganancia"] for p in pos]
    ren = [p["rentabilidad"] for p in pos]
    ...
    fig = go.Figure(
        go.Bar(
            ...
            text=[
                f'${g:,.0f}<br><span style="font-size:10px">{r:+.1f}%</span>'
                for g, r in zip(gan, ren)
            ],
            textposition="outside",
            textfont=dict(color=ct, size=12, family="DM Sans"),
            hovertemplate="<b>%{x}</b><br>$%{y:,.0f} COP<extra></extra>",
        )
    )
    fig.update_layout(
        title=dict(
            text="Ganancia por Activo (COP)",
            font=dict(size=13, color="#6e6e73", family="DM Sans"),
        ),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(17,17,17,0.6)",
        font=dict(color="#a1a1a6", family="DM Sans", size=12),
        height=320,
    ...
    fig.update_xaxes(
        ...
        color="#6e6e73",
        tickfont=dict(size=12, family="DM Sans", color="#6e6e73"),
    )
    fig.update_yaxes(
        gridcolor="rgba(255,255,255,0.04)",
        color="#6e6e73",
...
def grafica_evolucion(historico):
    ...
    vals = [r["resumen"]["total_valor"] for r in historico]
    invs = [r["resumen"]["total_invertido"] for r in historico]
    gans = [r["resumen"]["ganancia_total"] for r in historico]
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=fechas,
    ...
    fig.update_layout(
        title=dict(
            ...
            font=dict(size=13, color="#a1a1a6", family="DM Sans"),
        ),
        paper_bgcolor="rgba(0,0,0,0)",
        ...
        legend=dict(
            orientation="h", y=1.1, bgcolor="rgba(0,0,0,0)", font=dict(size=10)
        ),
        hovermode="x unified",
        yaxis=dict(
            title="Valor COP",
...
def nav_html(archivo, activa):
    def tab(href, label, nombre):
        s = (
            "background:#1c1c1e;color:#f5f5f7;border:1px solid rgba(255,255,255,0.1)"
            if activa == nombre
            else "background:transparent;color:#6e6e73;border:none"
        )
    ...
    return (
        ...
        f'border:1px solid rgba(255,255,255,0.08);flex-shrink:0;color:white">'
        + LOGO
        + "</div>"
...
def _dropdown_portafolios(archivo_actual):
    ...
    return (
        ...
        'box-shadow:0 8px 32px rgba(0,0,0,0.6)">' + opciones + "</div></div>"
    )


def header_portafolio(archivo, portafolio, perfil_badge, tiempo_real):
    g_color = (
        "#30d158" if tiempo_real and tiempo_real["ganancia_total"] > 0 else "#ff453a"
    )
    ...
    return (
        ...
        f'<a href="/logout" style="color:#6e6e73;font-size:12px;text-decoration:none">Salir</a>'
        "</div></div>"
        + nav_html(archivo, "dashboard").replace(
...
def mis_portafolios():
    ...
    if portafolios:
        for p in portafolios:
            ...
            borde = (
                "rgba(48,209,88,0.25)" if es_monitoreado else "rgba(255,255,255,0.07)"
            )
            fondo = (
                "rgba(48,209,88,0.04)" if es_monitoreado else "rgba(255,255,255,0.04)"
            )
            ...
            cards += (
                ...
                f'{p["nombre"]}'
                + (
                    ' <span style="font-size:11px;color:#30d158;font-weight:400">● en vivo</span>'
                    if es_monitoreado
                    else ""
                )
                + f"</div>"
    ...
    contenido = (
        ...
        + (
            f'<a href="/admin" class="btn btn-secondary" style="font-size:12px;padding:7px 14px">🛡 Admin</a>'
...
def login():
    ...
    if request.method == "POST":
        ...
        if usuario and not usuario.get("bloqueado"):
            ...
            try:
                threading.Thread(target=correr_todo, daemon=True).start()
            except Exception as e:
        ...
        if usuario and usuario.get("bloqueado"):
            ...
            registrar_actividad(
                ...
                email=email,
                detalle=f"Cuenta bloqueada — {minutos} min restantes",
                ip=ip,
                dispositivo=dispositivo,
            )
...
def nuevo_portafolio():
    ...
    if request.method == "POST":
        try:
            ...
            inv = float(
                request.form.get("inversion", "0").replace(",", "").replace(".", "")
                or 0
            )
            aporte = float(
                request.form.get("aporte", "0").replace(",", "").replace(".", "") or 0
            )
            freq = int(request.form.get("frecuencia", "1"))
            if not nombre or not propietario:
                error = "Nombre y propietario son obligatorios."
            else:
                r = crear_portafolio_para_usuario(
                    username, nombre, perfil, propietario, inv, aporte, freq
                )
                if r:
    ...
    link_html = (
        (
            '<div class="card"><a href="/mis-portafolios" class="btn btn-secondary" style="width:auto;display:inline-flex">Ver mis portafolios</a></div>'
        )
        if mensaje
        else ""
    )
...
def dashboard_portafolio(archivo):
    ...
    if composicion:
        fe = (
            "".join(
                f'<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;justify-content:space-between;align-items:center"><span style="color:#f5f5f7">{a}</span><span class="badge badge-green">ENTRADO</span></div>'
                for a in entrados_l
            )
            or '<div style="color:#6e6e73;padding:8px 0">Sin entradas aún</div>'
        )
        fp = (
            "".join(
                f'<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;justify-content:space-between;align-items:center"><span style="color:#f5f5f7">{a}</span><span class="badge badge-yellow">PENDIENTE</span></div>'
                for a in pendientes
            )
            or '<div style="color:#30d158;padding:8px 0">Ya entraste a todos</div>'
        )
        entradas_html = (
            '<div class="section-title">Estado de Entradas</div><div class="grid-2">'
    ...
    comp_html = (
        (
            f'<div class="section-title">Composición del Portafolio</div><div class="card">{g_torta}</div>'
        )
        if composicion
        else ""
    )
    ...
    if tiempo_real:
        gs = "+" if tiempo_real["ganancia_total"] > 0 else ""
        gc = "positivo" if tiempo_real["ganancia_total"] > 0 else "negativo"
        rs = "+" if tiempo_real["rentabilidad_total"] > 0 else ""
        rc = "positivo" if tiempo_real["rentabilidad_total"] > 0 else "negativo"
        filas = ""
        for pos in tiempo_real["posiciones"]:
            pg = "+" if pos["ganancia"] > 0 else ""
            pc = "positivo" if pos["ganancia"] > 0 else "negativo"
            pr = "+" if pos["rentabilidad"] > 0 else ""
            filas += (
                f'<tr><td><strong style="color:#f5f5f7">{pos["activo"]}</strong></td>'
        ...
        tr_html = (
            f'<div class="card" style="text-align:center;padding:40px"><p style="color:#6e6e73;margin-bottom:16px">Sin inversiones registradas.</p>'
        ...
        tr_html = (
            f'<div class="card" style="text-align:center;padding:40px"><p style="color:#6e6e73;margin-bottom:16px">Sin composición definida.</p>'
    ...
    if historico:
        ul = historico[-1]
        pr = historico[0]
        gac = ul["resumen"]["ganancia_total"]
        rac = ul["resumen"]["rentabilidad_total"]
        fh = ""
        for reg in reversed(historico):
            g = reg["resumen"]["ganancia_total"]
            r = reg["resumen"]["rentabilidad_total"]
            fh += (
                f'<tr><td>{reg["fecha"]}</td><td>${reg["macro"]["trm"]:,.0f}</td>'
...
def analista_view(archivo):
    ...
    if composicion:
        filas = "".join(
            f'<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06)"><span style="color:#f5f5f7">{a}</span><span class="badge badge-green">{v*100:.1f}%</span></div>'
            for a, v in composicion.items()
        )
        estado = (
            '<div class="alert alert-info" style="margin-bottom:16px">Ya tienes inversiones. Puedes crear un portafolio adicional.</div>'
            if tiene_inv
            else '<div class="alert alert-info" style="margin-bottom:16px">Tienes composición sin inversiones. Puedes reemplazarla o crear una adicional.</div>'
        )
        comp_html = (
            estado
            + f'<div class="card" style="margin-bottom:16px"><h3>Composición actual</h3>{filas}</div>'
        )

    contenido = (
        '<div class="container">'
        + nav_html(archivo, "analista")
        + "<h2>Analista de Portafolio</h2>"
        + comp_html
        ...
        f"  try{{"
        f'    const r=await fetch("/api/analista-chat/{archivo}",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{historial}})}});'
        f"    const data=await r.json(); const resp=data.respuesta;"
        f"    const cleaned=resp.trim();"
        f'    historial.push({{role:"assistant",content:cleaned}});'
        f"    let esJSON=false;"
        f"    function extraerJSON(txt){{"
        f'      const k = txt.indexOf("\\"accion\\":\\"analizar\\"");'
        f"      if(k===-1) return null;"
        f"      let start = k;"
        f'      while(start>0 && txt[start]!=="{{") start--;'
        f'      if(txt[start]!=="{{") return null;'
        f"      let depth = 0;"
        f"      for(let i=start; i<txt.length; i++){{"
        f'        if(txt[i]==="{{") depth++;'
        f'        else if(txt[i]==="}}"){{ depth--; if(depth===0) return txt.slice(start,i+1); }}'
        f"      }}"
        f"      return null;"
        f"    }}"
        f"    const jsonStr = extraerJSON(cleaned);"
        f"    if(jsonStr){{"
        f"      try{{"
        ...
        f"      if(d.propuesta){{"
        f"        window.propuestaActual=d.propuesta;"
        f"        iniciarEditor();"
        f"        const composicionReal = Object.entries(d.propuesta.pesos)"
        f"          .map(([k,v]) => `${{k}}: ${{(v*100).toFixed(1)}}%`).join(', ');"
        f'        historial.push({{role:"assistant", content:`Propuesta generada — composición real del portafolio: ${{composicionReal}}. Inversión: ${{d.propuesta.inversion}} COP. DCA: ${{d.propuesta.aporte_dca}} COP cada ${{d.propuesta.frecuencia_meses}} mes(es). Horizonte: ${{d.propuesta.horizonte}} años.`}});'
        f"      }}"
        f"    }}"
...
def seguimiento_view(archivo):
    ...
    if pendientes:
        ...
        pend_html = (
            '<div class="section-title">Pendientes por Entrar</div><div class="card">'
            + fp
            + "</div>"
        )
    ...
    if aportes:
        ...
        for i, a in enumerate(reversed(aportes)):
            ...
            precio_str = (
                f'${a.get("precio_usd", 0):,.4f}' if a.get("precio_usd") else "—"
            )
            monto_usd_str = (
                f'${a.get("monto_usd", 0):,.2f}' if a.get("monto_usd") else "—"
            )
            filas += (
                f"<tr>"
                f'<td>{a["fecha"]}</td>'
    ...
    pj = json.dumps(
        {
            ...
            "perfil": portafolio["perfil"],
            "propietario": portafolio["propietario"],
            "composicion": portafolio.get("composicion", {}),
        }
    )
    contenido = (
        '<div class="container">'
        + nav_html(archivo, "bot")
        + "<h2>Asistente de Inversión</h2>"
        '<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:18px;overflow:hidden">'
        '<div style="display:flex;align-items:center;gap:12px;padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.06);background:rgba(0,0,0,0.2)">'
        f'<div style="width:36px;height:36px;background:#0a0a0a;border-radius:10px;display:flex;align-items:center;justify-content:center;border:1px solid rgba(255,255,255,0.1);flex-shrink:0">{LOGO}</div>'
...
def monitor_view(archivo):
    ...
    if mercado_abierto():
        ...
        ahora = hora_colombia()
        msg_c = (
            "Reabre el lunes a las 9:30 AM"
            if ahora.weekday() >= 5
            else "Reabre mañana a las 9:30 AM"
        )
        mercado_badge = (
    ...
    if resultados:
        for r in sorted(resultados, key=lambda x: x["score"], reverse=True):
            s = r["senal"]
            col = {"ENTRAR": "#30d158", "VIGILAR": "#ffd60a", "NEUTRAL": "#6e6e73"}.get(
                s, "#6e6e73"
            )
            bg = {
                "ENTRAR": "rgba(48,209,88,0.06)",
                "VIGILAR": "rgba(255,214,10,0.06)",
                "NEUTRAL": "rgba(255,255,255,0.03)",
            }.get(s, "rgba(255,255,255,0.03)")
            bd = {
                "ENTRAR": "rgba(48,209,88,0.18)",
                "VIGILAR": "rgba(255,214,10,0.18)",
                "NEUTRAL": "rgba(255,255,255,0.07)",
            }.get(s, "rgba(255,255,255,0.07)")
            ...
            barra_c = (
                "#30d158"
                if r["score"] >= 6.5
                else "#ffd60a" if r["score"] >= 4.5 else "#3d3d3f"
            )
    ...
    if estado.get("justificacion") or estado.get("prediccion"):
        ...
        ia_html = (
            ...
            f'<div style="flex:1">'
            + (
                f'<p style="color:#a1a1a6;font-size:13px;line-height:1.6;margin:0 0 10px">{just}</p>'
                if just
                else ""
            )
    ...
    cierre_html = ""
    if estado.get("reporte_cierre") and estado.get(
        "reporte_cierre_fecha"
    ) == hora_colombia().strftime("%Y-%m-%d"):
        cierre_html = (
            '<div style="margin-top:20px">'
    ...
    config_html = (
        ...
        + (
            ...
            "</div>"
            if not portafolio.get("telegram_chat_id") and not chat_id_de(portafolio)
            else ""
        )
        + (
                ...
                if p["archivo"] != archivo:
                    ...
                    try:
                        ...
                        if otro.get("monitoreo_activo"):
                            ...
                            print(
                                f"🔴 Monitor desactivado en {p['archivo']} (exclusividad)"
                            )
                    except:
                        pass
    ...
    if request.method == "POST":
        ...
        if accion == "actualizar":
            try:
                ...
                if npw:
                    d["password_hash"] = hash_password_secure(npw)
                with open(ruta, "w", encoding="utf-8") as f:
                    json.dump(d, f, indent=2, ensure_ascii=False)
                portafolio = d
            ...
            activar_portafolio(archivo)
            mensaje = "Portafolio activado."
    msg_html = f'<div class="alert alert-success">{mensaje}</div>' if mensaje else ""
    err_html = f'<div class="alert alert-error">{error}</div>' if error else ""
    est = (
        '<span class="badge badge-green">ACTIVO para monitoreo</span>'
        if portafolio.get("activo")
        else '<span class="badge badge-gray">INACTIVO</span>'
    )
    btn_act = (
        ""
        if portafolio.get("activo")
        else '<form method="POST" style="margin-top:12px"><input type="hidden" name="accion" value="activar">'
        '<button type="submit" class="btn btn-secondary" style="width:auto">Activar para monitoreo</button></form>'
    )
    contenido = (
        '<div class="container" style="max-width:600px">'
        + nav_html(archivo, "config")
        + "<h2>Configuración</h2>"
        + msg_html
        + err_html
        + '<div class="card" style="margin-bottom:16px">'
...
def admin_panel():
    ...
    for u in usuarios.values():
        admin_badge = (
            '<span class="badge badge-yellow">ADMIN</span>' if u.get("es_admin") else ""
        )
